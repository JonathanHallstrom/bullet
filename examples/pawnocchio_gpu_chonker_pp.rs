...
use bullet_trainer::optimiser::OptimiserState;
use bytemuck::zeroed;
...
const NET_NAME: &'static str = "pawnocchio_gpu_chonker_pp";
...
fn main() {
    ...
    let warmup_sbs = 1;
    let warmup_batches = warmup_sbs * batches_per_superbatch;

    let pretrain_schedule = TrainingSchedule {
        net_id: NET_NAME.to_string() + "_pretrain",
        eval_scale: SCALE as f32,
        steps: TrainingSteps {
            batch_size,
            batches_per_superbatch,
            start_superbatch: 1,
            end_superbatch: PRETRAIN_SUPERBATCHES,
        },
        wdl_scheduler: wdl::ConstantWDL { value: 0.0 },
        lr_scheduler: lr::Sequence {
            first: lr::LinearDecayLR { initial_lr: 1e-5, final_lr: 1e-3, final_superbatch: PRETRAIN_SUPERBATCHES / 2 },
            second: lr::LinearDecayLR { initial_lr: 1e-3, final_lr: 1e-5, final_superbatch: PRETRAIN_SUPERBATCHES },
            first_scheduler_final_superbatch: PRETRAIN_SUPERBATCHES / 2,
        },
        save_rate: 25,
    };
    ...
    trainer.run(&pretrain_schedule, &settings, &loader);
